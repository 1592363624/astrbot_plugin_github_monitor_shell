...
- Issues 动态通知支持图片：新评论中的图片（GitHub 截图上传生成的 `<img src="...">` HTML 标签、markdown `![alt](url)` 语法、常见扩展名的裸图片直链）会在文本通知后以 QQ 图片消息补发，正文中原位置替换为「[图片]」占位符；单条通知最多补发 6 张，图片下载/发送失败仅记日志不影响文本通知
